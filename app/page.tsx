...
interface TwentyFourHourForecast {
  records: {
    general: {
      temperature: {
        low: number;
...
export default function SingaporeWeatherApp() {
  const [psiData, setPsiData] = useState<EnvironmentalData | null>(null)
  const [pm25Data, setPm25Data] = useState<EnvironmentalData | null>(null)
  const [twoHourForecast, setTwoHourForecast] = useState<WeatherForecast | null>(null)
  const [twentyFourHourForecast, setTwentyFourHourForecast] = useState<TwentyFourHourForecast | null>(null)
  const [uvIndex, setUVIndex] = useState<UVIndex | null>(null)
  const [selectedRegion, setSelectedRegion] = useState<string>('all')
  const [hoveredRegion, setHoveredRegion] = useState<string | null>(null);
  // const [mousePosition, setMousePosition] = useState<{ x: number; y: number }>({ x: 0, y: 0 }); // Track mouse position


  useEffect(() => {
    const fetchData = async () => {
      try {
        const [psiResponse, pm25Response, twoHourResponse, twentyFourHourResponse, uvResponse] = await Promise.all([
        ...
        const psiJson = await psiResponse.json()
        const pm25Json = await pm25Response.json()
        const twoHourJson = await twoHourResponse.json()
        const twentyFourHourJson = await twentyFourHourResponse.json()
        const uvJson = await uvResponse.json()
        setPsiData(psiJson.data)
        setPm25Data(pm25Json.data)
        setTwoHourForecast(twoHourJson.data)
        setTwentyFourHourForecast(twentyFourHourJson.data)
        setUVIndex(uvJson.data)
      } catch (error) {
  ...
  const getPSIColor = (psi: number) => {
    if (psi <= 50) return "bg-green-100 text-green-800"
    if (psi <= 100) return "bg-yellow-100 text-yellow-800"
    if (psi <= 200) return "bg-orange-100 text-orange-800"
    if (psi <= 300) return "bg-red-100 text-red-800"
    return "bg-purple-100 text-purple-800"
  }

  const getPM25Color = (pm25: number) => {
    if (pm25 <= 12) return "bg-green-100 text-green-800"
    if (pm25 <= 35) return "bg-yellow-100 text-yellow-800"
    if (pm25 <= 55) return "bg-orange-100 text-orange-800"
    if (pm25 <= 150) return "bg-red-100 text-red-800"
    return "bg-purple-100 text-purple-800"
  }
  ...
  const getRegion = (location: string): string => {
    // First check in the manual mapping
    if (regionMapping[location]) {
      return regionMapping[location];
    }
  
    // Fallback logic if not explicitly mapped
    if (location.toLowerCase().includes('east')) return 'east';
    if (location.toLowerCase().includes('west')) return 'west';
    if (location.toLowerCase().includes('north')) return 'north';
    if (location.toLowerCase().includes('south')) return 'south';
    if (location.toLowerCase().includes('central')) return 'central';
  
    return 'unknown';
  };
  
  // Example usage inside your component:
  const filteredForecasts = twoHourForecast?.items[0].forecasts.filter(forecast => {
    const region = getRegion(forecast.area);
      ...
      <div className="max-w-6xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden">
        <div className="p-6">
          <div className="flex justify-between items-center mb-6">
            <h1 className="text-3xl font-bold">SG Environment Feed [LIVE] </h1>
          </div>
          <div className="gap-8">
            {/* PSI and PM2.5 Data */}
            {/* {psiData && pm25Data && regions.map(region => (
              (selectedRegion === 'all' || selectedRegion === region) && (
                <div key={region} className="bg-white rounded-lg shadow p-4">
                  <h2 className="text-xl font-semibold mb-4 capitalize">{region}</h2>
                  <div className={`mb-2 p-2 rounded ${getPSIColor(psiData.items[0].readings.psi_twenty_four_hourly[region])}`}>
                    PSI: {psiData.items[0].readings.psi_twenty_four_hourly[region]}
                  </div>
                  <div className={`p-2 rounded ${getPM25Color(pm25Data.items[0].readings.pm25_one_hourly[region])}`}>
                    PM2.5: {pm25Data.items[0].readings.pm25_one_hourly[region]}
                  </div>
                  <p className="text-xs text-gray-500 mt-2">
                    Last updated: {formatDateTime(psiData.items[0].updatedTimestamp)}
                  </p>
                </div>
              )
            ))} */}
            ...
            {/* SVG Map with Hover */}
            <div className="relative mb-8" style={{ width: '100%', height: '60%' }}>
            <svg
            ...
            >
              ...
              className="fill-current text-gray-300 hover:text-blue-300 transition duration-300 ease-in-out"
              ...
              className="fill-current text-gray-300 hover:text-blue-300 transition duration-300 ease-in-out"
              onMouseEnter={() => handleRegionHover('south')}
              onMouseLeave={() => handleRegionHover(null)}
              style={{
                stroke: "black", /* Set outline color */
                strokeWidth: 1, /* Set outline thickness */
              ...
              className="fill-current text-gray-300 hover:text-blue-300 transition duration-300 ease-in-out"
              onMouseEnter={() => handleRegionHover('north')}
              onMouseLeave={() => handleRegionHover(null)}
              style={{
                stroke: "black", /* Set outline color */
                strokeWidth: 1, /* Set outline thickness */
            ...
            {/* Hovered Region Data Card */}
            {hoveredRegion && psiData && pm25Data && (
              <div
              ...
              >
                <h2 className="text-xl font-semibold mb-4 capitalize">{hoveredRegion}</h2>
                <div className={`mb-2 p-2 rounded ${getPSIColor(psiData.items[0].readings.psi_twenty_four_hourly[hoveredRegion])}`}>
                  PSI: {psiData.items[0].readings.psi_twenty_four_hourly[hoveredRegion]}
                </div>
                <div className={`p-2 rounded ${getPM25Color(pm25Data.items[0].readings.pm25_one_hourly[hoveredRegion])}`}>
                  PM2.5: {pm25Data.items[0].readings.pm25_one_hourly[hoveredRegion]}
                </div>
                <p className="text-xs text-gray-500 mt-2">
                  Last updated: {formatDateTime(psiData.items[0].updatedTimestamp)}
                </p>
            ...
            <div className="bg-yellow-50 rounded-lg p-4 lg:col-span-3">
            <div className = "flex justify-between items-center mb-6">
              <h2 className="text-xl font-semibold mb-4">2-Hour Weather Forecast</h2>
              <select 
            ...
            >
              ...
              {filteredForecasts && (
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
                  {filteredForecasts.map((forecast) => (
                    <div key={forecast.area} className="bg-white rounded-lg shadow p-4">
                      <h3 className="font-semibold mb-2">{forecast.area}</h3>
                      <div className="flex items-center">
                        <span className="mr-2 text-2xl" aria-hidden="true">{getWeatherIcon(forecast.forecast)}</span>
              ...
              {twoHourForecast && (
                <p className="text-xs text-gray-500 mt-4">
                  Last updated: {formatDateTime(twoHourForecast.items[0].update_timestamp)}
                </p>
              )}
            </div>

            {/* 24-Hour Weather Forecast */}
            <div className="bg-green-50 rounded-lg p-4 col-span-full">
              <h2 className="text-xl font-semibold mb-4">24-Hour Weather Forecast</h2>
              {twentyFourHourForecast && (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  <div className="bg-white rounded-lg shadow p-4 flex items-center">
                    <Thermometer className="w-6 h-6 mr-2 text-red-500" aria-hidden="true" />
                    <span>Temperature: {twentyFourHourForecast.records[0].general.temperature.low}°C - {twentyFourHourForecast.records[0].general.temperature.high}°C</span>
                  </div>
                  <div className="bg-white rounded-lg shadow p-4 flex items-center">
                    <Droplets className="w-6 h-6 mr-2 text-blue-500" aria-hidden="true" />
                    <span>Humidity: {twentyFourHourForecast.records[0].general.relativeHumidity.low}% - {twentyFourHourForecast.records[0].general.relativeHumidity.high}%</span>
                  </div>
                  <div className="bg-white rounded-lg shadow p-4 flex items-center">
                    <Wind className="w-6 h-6 mr-2 text-gray-500" aria-hidden="true" />
                    <span>Wind: {twentyFourHourForecast.records[0].general.wind.speed.low} - {twentyFourHourForecast.records[0].general.wind.speed.high} km/h, {twentyFourHourForecast.records[0].general.wind.direction}</span>
                  </div>
                </div>
              )}
              {twentyFourHourForecast && (
                <p className="text-xs text-gray-500 mt-4">
                  Last updated: {formatDateTime(twentyFourHourForecast.records[0].timestamp)}
                </p>
              )}
            </div>
            ...
            <div className="bg-blue-50 rounded-lg p-4 col-span-full">
              <h2 className="text-xl font-semibold mb-4">UV Index</h2>
              {uvIndex && (
                <div className="h-64">
                  <ResponsiveContainer width="100%" height="100%">
                    <LineChart data={uvIndex.records[0].index.slice().reverse()}>
                      <CartesianGrid strokeDasharray="3 3" />
                      <XAxis 
                        dataKey="hour" 
                        tickFormatter={(value) => new Date(value).toLocaleTimeString('en-SG', { hour: '2-digit', minute: '2-digit', timeZone: 'Asia/Singapore' })}
                      />
                      <YAxis />
                      <Tooltip 
                        labelFormatter={(value) => new Date(value).toLocaleString('en-SG', { timeZone: 'Asia/Singapore' })}
              ...
              {uvIndex && (
                <p className="text-xs text-gray-500 mt-4">
                  Last updated: {formatDateTime(uvIndex.records[0].updatedTimestamp)}
                </p>
              )}
            </div>
          </div>
